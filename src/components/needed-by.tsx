...
export function NeededBySignal({ date, riskDays, status, compact }: { date: string | null; riskDays: number; status: string; compact?: boolean }) {
  ...
  const textCls = cn("text-xs", d < 0 ? "text-rose-600" : d <= riskDays ? "text-amber-700" : "text-slate-400");
  if (compact) {
    return (
      <span className="inline-flex items-start gap-1.5">
        <span className={cn("mt-1.5 size-2.5 shrink-0 rounded-full", tone)} aria-hidden />
        <span className="flex flex-col">
          <span className="whitespace-nowrap">{formatDate(date)}</span>
          {text && <span className={cn(textCls, "whitespace-nowrap")}>{text}</span>}
        </span>
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1.5 whitespace-nowrap">
      ...
      {formatDate(date)}
      {text && <span className={textCls}>({text})</span>}
    </span>
  );
}
